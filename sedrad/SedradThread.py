...
from .messages_pb2 import SedradMessage


MAX_MESSAGE_LENGTH = 1024 * 1024 * 1024  # 1GB


class SedradCommunicationError(Exception): pass


# pipenv run python -m grpc_tools.protoc -I./protos --python_out=. --grpc_python_out=. ./protos/rpc.proto ./protos/messages.proto ./protos/p2p.proto

class SedradThread(object):
    def __init__(self, sedrad_host, sedrad_port, async_thread=True):

        self.sedrad_host = sedrad_host
        self.sedrad_port = sedrad_port

        if async_thread:
            self.channel = grpc.aio.insecure_channel(f'{sedrad_host}:{sedrad_port}',
            ...
            self.channel = grpc.insecure_channel(f'{sedrad_host}:{sedrad_port}',
                                                 compression=grpc.Compression.Gzip,
                                                 options=[
                                                     ('grpc.max_send_message_length', MAX_MESSAGE_LENGTH),
                                                     ('grpc.max_receive_message_length', MAX_MESSAGE_LENGTH),
    ...
    async def request(self, command, params=None, wait_for_response=True, timeout=120):
        if wait_for_response:
            try:
                ...
                raise SedradCommunicationError(str(e))
